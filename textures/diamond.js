...
float diamond(in vec3 point){
	...
	if((dist<0.0&&!inside_object)||(dist>0.0&&inside_object)){
		...
		if(!inside_object&&!object_entered){
			...
			reflection_color=textureCube(iChannel0,reflection_direction);
		}else{
			float transmit_0=dot(direction_vector,normal);
			float transmit_1=transmittance*transmit_0;
			vec4 temp_color=vec4(0.0);
			temp_color.r=textureCube(iChannel0,direction_vector_r+normal*(1.0-DISPERSION)).r;
			temp_color.g=textureCube(iChannel0,direction_vector+normal).g;
			temp_color.b=textureCube(iChannel0,direction_vector_b+normal*(1.0+DISPERSION)).b;
...
vec4 trace_ray(){
	...
	for(int i=0;i<128;i++){
		...
		if(length(point)>10.0){
			color=textureCube(iChannel0,direction_vector);
			break;
		}
